...
Task 027 enforces single-weapon physical ownership by removing the temporary 1/2 debug selection inputs and gameplay paths. The automatic rifle remains equipped at startup, and physical F pickup swaps preserve each weapon's stored ammunition. WeaponController starts reload automatically after an emptying shot or when an equipped weapon has zero loaded ammunition with reserve remaining; no reload starts when reserve ammunition is zero.
